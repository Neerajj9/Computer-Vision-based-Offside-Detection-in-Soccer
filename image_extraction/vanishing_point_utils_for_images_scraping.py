    ...
    while linesFound == False:
        for line in lines: 
            for r,theta in line:
                ...
                # (theta * 180 * 7 / 22) < angleMinLimit or 
                if (theta * 180 * 7 / 22) > angleMaxLimit:
                    x0 = a*r 
                    y0 = b*r 
                    x1 = int(x0 + 1000*(-b)) 
                    y1 = int(y0 + 1000*(a)) 
                    ...
                    #cv2.putText(image, str((theta * 180 * 7 / 22)) ,(int((x1 + x2)/2) + np.random.randint(0,10) ,  int((y1+y2)/2)) + np.random.randint(0,10) , cv2.FONT_HERSHEY_SIMPLEX, 1, (200,255,155), 2, cv2.LINE_AA)
        ...
        if len(selectedLines) < 2:
            if rLimit >= 75:
                ...
                angleMaxLimit += 1
                rlimit = 100
                if angleMinLimit == 0: return []
                if angleMaxLimit == 180: return []
        else:
...
def get_vertical_vanishing_point(img):
    
    selectedLines = get_vertical_lines(img)
...
def get_ground_plane(img, vertical_vanishing_point, horizontal_vanishing_point):
    allVerticalLines = get_vertical_lines(img)
